...
sub runCnsFec($$) {
    ...
    my $rawreads = "$prjDir/1-consensus/raw_reads/filtered_raw_reads.fasta";
    my $rd_2_rd = "$prjDir/1-consensus/ovlp.paf";
    ...
    my $job0 = Job->new(
        ...
        ofiles => [$rd_2_rd],
        mfiles => [],
        cmds => ["minimap2 -x ava-ont -w 20 -K 2g -f 0.005 -t $threads  $rawreads $rawreads | awk '{ if(\$4 - \$3 >= 0.2 * \$2 || \$9 - \$8 >= 0.2 * \$7) print \$0}' > $rd_2_rd"],
        msg => "fec_job0",
    );
    ...
    my $job1 = Job->new(
        ...
        ofiles => [$rd_2_rd],
        mfiles => [],
        cmds => ["Fec -x 1 -t $threads -r 0.6 -a 400 -c 0 -l 1000 -m 0.005 -f 0.2 $rd_2_rd $rawreads corrected1.fasta"],
        msg => "fec_job1",
    );
...
sub runCnsIterate_with_FEC($$$) {
    ...
    my $input = "$prjDir/1-consensus/raw_reads/filtered_raw_reads.fasta";
    if ($step > 1) {
        $input = "$prjDir/1-consensus/cns_iter" . ($step -1) . "/cns.fasta";
    }

    my $rd_2_rd = "$workDir/ovlp.paf";
    my $output = "$workDir/cns.fasta";

    my $threads = %$cfg{"THREADS"};

    my $job0 = Job->new(
        name => "cns_fec_${step}_job0",
        ifiles => [$input],
        ofiles => [$rd_2_rd],
        mfiles => [],
        cmds => ["minimap2 -x ava-ont -w 20 -K 2g -f 0.005 -t $threads  $input $input | awk '{ if(\$4 - \$3 >= 0.2 * \$2 || \$9 - \$8 >= 0.2 * \$7) print \$0}' > $rd_2_rd"],
        msg => "fec_job0",
    );

    my $job1 = Job->new(
        name => "cns_fec_${step}_job1",
        ifiles => [$rd_2_rd, $input],
        ofiles => [$output],
        mfiles => [],
        cmds => ["Fec -x 1 -t $threads -r 0.6 -a 400 -c 0 -l 1000 -m 0.005 -f 0.2 $rd_2_rd $input $output"],
        msg => "fec_job1",
    );
...
#     my $rawreadList = %$cfg{"ONT_READ_LIST"};
#     print("--xx",  $rawreadList, "xx\n");

#     sub funcRawPrepare0($$) {
#         my ($env, $cfg) = @_;
#         print("--",  $rawreadList, "xx\n");
...
#         mfiles => [],
#         funcs => [\&funcRawPrepare0, \&runCnsFec, \&funcRawExtract], #prefunc
...
sub runConsensusRaw_with_FEC($$) {
    ...
    my $rawreadList = %$cfg{"ONT_READ_LIST"};
    my @funcs = (sub ($$) { runCnsIterate_with_FEC($env, $cfg, 1)},
                 sub ($$) { runCnsIterate_with_FEC($env, $cfg, 2)},
                 sub ($$) { runCnsIterate_with_FEC($env, $cfg, 3)},
                 sub ($$) { runCnsIterate_with_FEC($env, $cfg, 4)},
                 sub ($$) { runCnsIterate_with_FEC($env, $cfg, 5)},
                 sub ($$) { runCnsIterate_with_FEC($env, $cfg, 6)},
                 sub ($$) { runCnsIterate_with_FEC($env, $cfg, 7)},
                 sub ($$) { runCnsIterate_with_FEC($env, $cfg, 8)});
    ...
    sub funcRawExtract($$) {
        my ($env, $cfg) = @_;
        my $count = %$cfg{"NUM_ITER"};
        my $lastCnsReads = "$workDir/cns_iter$count/cns.fasta"; 
        runCnsExtract($env, $cfg, $lastCnsReads);
...
sub runConsensus($$) {
    ...
    if (not exists($cfg->{"CNS_READ_LIST"}) or %$cfg{"CNS_READ_LIST"} eq "") {
        runConsensusRaw_with_FEC($env, $cfg);
    } else {
